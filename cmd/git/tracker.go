...
func trackerFunc(cmd *cobra.Command, args []string) error {
	...
	switch {
	case tkFlags.status:
		return printer.GitRepoTracked(f, g)
	case tkFlags.mgt:
...
func trackExportCommit(t *terminal.Term, f *frame.Frame, g *git.Manager) error {
	if !g.IsInitialized() {
...
func initGPGRepo(g *git.Manager) error {
	gr := g.Tracker.Current()
	...
	if err := port.GitExport(g); err != nil {
		if errors.Is(err, git.ErrGitNothingToCommit) {
			wmsg := fmt.Sprintf("Skipping %q, no bookmarks found\n", gr.DBName)
			fmt.Print(txt.WarningMesg(wmsg))
			return nil
		}
...
func initJSONRepo(g *git.Manager) error {
	if err := port.GitExport(g); err != nil {
		if errors.Is(err, git.ErrGitNothingToCommit) {
			wmesg := fmt.Sprintf("Skipping %q, no bookmarks found\n", g.Tracker.Current().DBName)
...
func initTracking(g *git.Manager) error {
	if gpg.IsInitialized(g.RepoPath) {
		return initGPGRepo(g)
	}

	return initJSONRepo(g)
}
...
func untrackDropCommit(t *terminal.Term, f *frame.Frame, g *git.Manager) error {
	...
	t.ReplaceLine(1, f.Reset().Warning(fmt.Sprintf("Untracking %q", gr.Name)).String())
...
func managementSelect(t *terminal.Term, f *frame.Frame, g *git.Manager) ([]string, error) {
	...
	for _, dbPath := range dbFiles {
		...
		if !t.Confirm(f.Reset().Question(fmt.Sprintf("Track %q?", gr.Name)).String(), "n") {
			t.ReplaceLine(1, f.Reset().Warning(fmt.Sprintf("Skipping %q", gr.Name)).String())
			continue
		}
		...
		t.ReplaceLine(1, f.Reset().Success(fmt.Sprintf("Tracking %q", gr.Name)).String())
	}
...
func management(t *terminal.Term, f *frame.Frame, g *git.Manager) error {
	dbFiles, err := files.Find(config.App.Path.Data, "*.db")
	if err != nil {
		return fmt.Errorf("finding db files: %w", err)
	}

	f.Header("Tracked database management\n").Rowln().Flush()
	...
	for _, dbPath := range dbFiles {
		...
		if err := untrackDropCommit(t, f.Reset(), g); err != nil {
			return err
		}
	}
